...
**Running the E2E suite in WSL without sudo.** Playwright's Chromium (`~/.cache/ms-playwright/chromium_headless_shell-*`) needs `libnspr4`, `libnss3` and `libasound2t64`, which the distro doesn't have, and `playwright install-deps` needs root. Extract them into a user dir instead: `mkdir -p ~/pwlibs/debs && cd ~/pwlibs/debs && apt-get download libnspr4 libnss3 libasound2t64`, then `for f in *.deb; do dpkg -x "$f" ~/pwlibs/root; done`, and run pytest with `LD_LIBRARY_PATH=$HOME/pwlibs/root/usr/lib/x86_64-linux-gnu DJANGO_SETTINGS_MODULE=koala_budget.settings_e2e DJANGO_DATABASE_PASSWORD=postgres uv run pytest e2e/` (after `uv sync --group e2e`, with Vite running on 5173). Check with `ldd <chrome-headless-shell> | grep "not found"`. The full suite takes about 7–9 minutes.
