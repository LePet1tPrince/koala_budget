...
class SampleApplyTest(TestCase):
    ...
    def test_everything_was_written(self):
        ...
        self.assertEqual(Account.objects.filter(team=self.team).count(), 104 + 3)  # + one per goal
        self.assertEqual(Budget.objects.filter(team=self.team).count(), 1872)
        self.assertEqual(Goal.objects.filter(team=self.team).count(), 3)
