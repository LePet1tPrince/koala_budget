...
class TinyApplyTest(TestCase):
    ...
    def setUp(self):
        ...
        self.result = apply_plan(self.team, self.plan, user=self.user)

    def account(self, name):
        return Account.objects.get(team=self.team, name=name)

    def test_the_chart_of_accounts(self):
        types = {
            (account.name, account.account_group.account_type)
            for account in Account.objects.filter(team=self.team).select_related("account_group")
        }
    ...
    def test_every_entry_is_posted_and_balanced(self):
        entries = JournalEntry.objects.filter(team=self.team)
    ...
    def test_net_worth_is_what_the_user_actually_has(self):
        net_worth = NetWorthService(self.team).get_net_worth(date(2024, 3, 1))
        self.assertEqual(net_worth, Decimal("2291.25"))

    def test_reconciliation_flags_land_on_the_bank_side_only(self):
        entry = JournalEntry.objects.get(team=self.team, description="Weekly shop")
    ...
    def test_goals_arrive_funded_but_not_complete(self):
        goal = Goal.objects.get(team=self.team, name="House")
        ...
        self.assertEqual(
            GoalAllocation.objects.get(team=self.team, goal=goal, month=date(2024, 1, 1)).amount,
            Decimal("300.00"),
        )

    def test_opening_balances_are_dated_when_the_account_opened(self):
        opening = JournalEntry.objects.get(team=self.team, description="Opening balance — Chequing")
    ...
    def test_a_card_balance_opens_as_money_owed(self):
        opening = JournalEntry.objects.get(team=self.team, description="Opening balance — Visa")
        lines = {line.account.name: line for line in opening.lines.all()}
    ...
    def test_no_bank_feed_rows_are_created(self):
        ...
        self.assertEqual(BankTransaction.objects.filter(team=self.team).count(), 0)

    def test_a_second_import_is_refused(self):
        self.assertFalse(can_import(self.team))
        with self.assertRaisesMessage(ApplyError, "already has transactions"):
            apply_plan(self.team, build(tiny_analysis()), user=self.user)
...
class SkippedAccountApplyTest(TestCase):
    def test_a_dropped_account_is_not_created_and_its_rows_do_not_appear(self):
        ...
        apply_plan(team, plan, user=user)

        self.assertFalse(Account.objects.filter(team=team, name="Savings").exists())
        self.assertTrue(all(entry.is_balanced for entry in JournalEntry.objects.filter(team=team)))
...
class ExistingChartApplyTest(TestCase):
    def test_an_import_over_a_stock_chart_reuses_its_groups(self):
        ...
        apply_template(team=team, template=PERSONAL_BUDGET_TEMPLATE)
        before = AccountGroup.objects.filter(team=team).count()

        analysis = tiny_analysis()
        apply_plan(team, build(analysis), user=user)

        self.assertEqual(AccountGroup.objects.filter(team=team, name="Bank Accounts").count(), 1)
        self.assertGreater(AccountGroup.objects.filter(team=team).count(), before - 1)
        self.assertTrue(Account.objects.filter(team=team, name="Chequing").exists())
...
class SampleApplyTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.result = apply_plan(cls.team, cls.plan, user=cls.user)
    ...
    def test_everything_was_written(self):
        ...
        self.assertEqual(JournalEntry.objects.filter(team=self.team).count(), 6623 + 13)
        self.assertEqual(JournalLine.objects.filter(team=self.team).count(), 13335 + 13 * 2)
        ...
        self.assertEqual(Account.objects.filter(team=self.team).count(), 106)
        # No `Budget` rows on goal categories: 56 of the old 1,872 were on them.
        self.assertEqual(Budget.objects.filter(team=self.team).count(), 1816)
        self.assertEqual(Goal.objects.filter(team=self.team).count(), 4)

    def test_every_goal_has_its_account_outside_the_system_group(self):
        goals = Goal.objects.filter(team=self.team).select_related("account__account_group")
    ...
    def test_spending_from_a_savings_category_lands_on_the_goal(self):
        goal_lines = JournalLine.objects.filter(team=self.team, account__goal__isnull=False).count()
        self.assertEqual(goal_lines, 24)
        self.assertFalse(Budget.objects.filter(team=self.team, category__goal__isnull=False).exists())

    def test_a_spent_out_savings_category_arrives_closed_with_its_history(self):
        house = Goal.objects.get(team=self.team, name="House")
    ...
    def test_net_worth_matches_the_preview(self):
        net_worth = NetWorthService(self.team).get_net_worth(date(2026, 10, 1))
    ...
    def test_budget_available_matches_ynab_for_a_sampled_category(self):
        ...
        for group, name in (("Monthly", "Groceries"), ("Tax Deductible", "Medical")):
            account = Account.objects.get(team=self.team, name=name, account_group__account_type="expense")
            expected = next(
    ...
    def test_transactions_carry_their_payees(self):
        ...
        # does.
        self.assertGreater(JournalEntry.objects.filter(team=self.team, payee__isnull=False).count(), 5000)
        self.assertEqual(
            JournalEntry.objects.filter(
                team=self.team, payee__isnull=False, description__startswith="Opening balance"
            ).count(),
            0,
        )

    def test_reconciled_lines_are_the_bank_side(self):
        reconciled = JournalLine.objects.filter(team=self.team, is_reconciled=True).select_related(
