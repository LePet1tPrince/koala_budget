...
class AccountHealthTests(TestCase):
    ...
    def test_earlier_months_unreconciled_backlog_raises_no_flag(self):
        ...
        entry = JournalEntry.objects.create(
            book=self.book, entry_date=date(2026, 7, 3), description="Manual", status="posted"
        )
        ...
        txn = BankTransaction.objects.create(
            book=self.book, account=account, amount=Decimal("1.00"), posted_date=date(2026, 8, 25), description="x"
        )
    ...
    def test_counts_cover_only_the_month_and_unreconciled_never_exceeds_transactions(self):
        ...
        for day in (2, 9, 16):
            july = BankTransaction.objects.create(
                book=self.book, account=account, amount=Decimal("5.00"), posted_date=date(2026, 7, day), description="J"
            )
            self._categorize(july, category)
        august = BankTransaction.objects.create(
            book=self.book, account=account, amount=Decimal("5.00"), posted_date=date(2026, 8, 4), description="A"
        )
        self._categorize(august, category)
        BankTransaction.objects.create(
            book=self.book, account=account, amount=Decimal("5.00"), posted_date=date(2026, 8, 6), description="New"
        ...
        manual = JournalEntry.objects.create(
            book=self.book, entry_date=date(2026, 8, 8), description="Manual", status="posted"
        )
        JournalLine.objects.create(
            book=self.book, journal_entry=manual, account=account, dr_amount=Decimal("1"), is_reconciled=True
        )
        JournalLine.objects.create(book=self.book, journal_entry=manual, account=category, cr_amount=Decimal("1"))
    ...
    def test_balance_change_is_movement_within_the_month(self):
        ...
        category = Account.objects.create(book=self.book, name="Groceries", account_group=self.equity_group)
        july = BankTransaction.objects.create(
            book=self.book, account=account, amount=Decimal("100.00"), posted_date=date(2026, 7, 15), description="Jul"
        )
        self._categorize(july, category, reconciled=True)
        august = BankTransaction.objects.create(
            book=self.book, account=account, amount=Decimal("30.00"), posted_date=date(2026, 8, 10), description="Aug"
        )
        self._categorize(august, category, reconciled=True)
    ...
    def test_institution_name_is_reported(self):
        ...
        account = self._account("Chequing")
        account.institution = Institution.objects.create(book=self.book, name="Maple Bank")
        account.save()
        row = account_health(self.book, self.month)["accounts"][0]
