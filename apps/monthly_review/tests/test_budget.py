...
class BudgetBreakdownTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.bank = AccountGroup.objects.create(team=cls.team, name="Cash", account_type="asset")
        cls.chequing = Account.objects.create(team=cls.team, name="Chequing", account_group=cls.bank)
        ...
        cls.other = Account.objects.create(team=cls.team, name="Miscellaneous", account_group=cls.expense_group)
    ...
    def _spend(self, category, amount, day):
        entry = JournalEntry.objects.create(team=self.team, entry_date=day, description="spend", status="posted")
    ...
    def _budget(self, category, month, amount):
        Budget.objects.create(team=self.team, category=category, month=month, budget_amount=Decimal(amount))
    ...
    def test_overspent_category_with_real_carried_over_balance(self):
        ...
        breakdown = budget_breakdown(self.team, date(2026, 8, 1))
    ...
    def test_groups_ordered_income_first(self):
        ...
        entry = JournalEntry.objects.create(
            team=self.team, entry_date=date(2026, 8, 1), description="pay", status="posted"
        )
        JournalLine.objects.create(
            team=self.team, journal_entry=entry, account=self.chequing, dr_amount=Decimal("3000")
        )
        JournalLine.objects.create(team=self.team, journal_entry=entry, account=self.salary, cr_amount=Decimal("3000"))
