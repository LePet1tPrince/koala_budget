...
class BuildReviewTests(TestCase):
    ...
    def _entry(self, day, category, amount, *, dr_category=True, status=JournalEntry.STATUS_POSTED):
        entry = JournalEntry.objects.create(book=self.book, entry_date=day, description="test", status=status)
        if dr_category:
            ...
            JournalLine.objects.create(book=self.book, journal_entry=entry, account=category, cr_amount=amount)
            JournalLine.objects.create(book=self.book, journal_entry=entry, account=self.chequing, dr_amount=amount)
        return entry

    def test_payload_top_level_keys(self):
        review = build_review(self.book, date(2026, 8, 1))
    ...
    def test_month_before_first_activity_returns_empty_state(self):
        ...
        review = build_review(self.book, date(2025, 1, 1))
    ...
    def test_net_worth_bands_combine_feed_accounts_and_keep_other_groups(self):
        liability_group = AccountGroup.objects.create(book=self.book, name="Cards", account_type="liability")
        loan_group = AccountGroup.objects.create(book=self.book, name="Loans", account_type="liability")
        card = Account.objects.create(book=self.book, name="Visa", account_group=liability_group, has_feed=True)
        loan = Account.objects.create(book=self.book, name="Car loan", account_group=loan_group)

        def post(day, dr, cr, amount):
            entry = JournalEntry.objects.create(book=self.book, entry_date=day, description="t", status="posted")
            JournalLine.objects.create(book=self.book, journal_entry=entry, account=dr, dr_amount=amount)
            JournalLine.objects.create(book=self.book, journal_entry=entry, account=cr, cr_amount=amount)
        ...
        stack = build_review(self.book, date(2026, 8, 1))["net_worth"]["stack"]
