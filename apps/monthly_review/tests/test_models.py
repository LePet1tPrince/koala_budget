...
class MonthlyReviewStateTests(TestCase):
    ...
    def test_month_normalized_to_first_of_month(self):
        state = MonthlyReviewState.objects.create(team=self.team, month=date(2026, 8, 15))
        self.assertEqual(state.month, date(2026, 8, 1))

    def test_is_finished_false_by_default(self):
        state = MonthlyReviewState.objects.create(team=self.team, month=date(2026, 8, 1))
    ...
    def test_unique_together_team_month(self):
        MonthlyReviewState.objects.create(team=self.team, month=date(2026, 8, 1))
