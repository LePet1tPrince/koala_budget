...
class ReportService:
    ...
    def goal_account_ids(self):
        ...
        return set(Account.objects.filter(team=self.team, goal__isnull=False).values_list("pk", flat=True))
    ...
    def _goal_chart_data(self, account, start_date, end_date):
        ...
        allocated = {
            row["month"]: row["total"]
            for row in GoalAllocation.objects.filter(team=self.team, goal__account=account, month__lt=month_after_last)
            .values("month")
        ...
        spent = {
            ...
            for row in JournalLine.objects.filter(
                team=self.team, account=account, journal_entry__entry_date__lt=month_after_last
            )
            .filter(counted_entries("journal_entry__"))
            .annotate(month=TruncMonth("journal_entry__entry_date"))
            .values("month")
