...
class NavSubItemActiveTest(NavTestBase):
    def test_monthly_review_shades_its_sub_item_not_reports(self):
        response = self.client.get(reverse("monthly_review:home", args=[self.team.slug]))
        self.assertEqual(response.context["nav_item"], "monthly-review")
        nav = self._nav(reverse("monthly_review:home", args=[self.team.slug]))
        self.assertIn("nav-monthly-review", _active_testids(nav))
        reports_url = reverse("reports:reports_home", args=[self.team.slug])
    ...
    def test_each_report_shades_only_its_own_sub_item(self):
        ...
        for url_name, testid in cases.items():
            with self.subTest(url_name=url_name):
                self.assertEqual(_active_testids(self._nav(reverse(url_name, args=[self.team.slug]))), {testid})

    def test_reports_home_shades_the_parent(self):
        url = reverse("reports:reports_home", args=[self.team.slug])
        nav = self._nav(url)
    ...
    def test_nav_lists_every_report_on_the_reports_page(self):
        nav = self._nav(reverse("reports:reports_home", args=[self.team.slug]))
        for url_name in ("reports:dollar_map", "reconciliation:hub"):
            with self.subTest(url_name=url_name):
                self.assertIn(reverse(url_name, args=[self.team.slug]), nav)
    ...
    def test_balance_sheet_drill_down_shades_balance_sheet(self):
        with current_team(self.team):
            group = AccountGroup.objects.create(team=self.team, name="Cash", account_type=ACCOUNT_TYPE_ASSET)
            account = Account.objects.create(team=self.team, name="Checking", account_group=group)
        url = reverse("reports:account_activity", args=[self.team.slug, account.id]) + "?source=balance_sheet"
...
class NavInboxBalanceTest(NavTestBase):
    ...
    @classmethod
    def _seed(cls):
        assets = AccountGroup.objects.create(team=cls.team, name="Cash", account_type=ACCOUNT_TYPE_ASSET)
        expenses = AccountGroup.objects.create(team=cls.team, name="Food", account_type=ACCOUNT_TYPE_EXPENSE)
        cls.checking = Account.objects.create(team=cls.team, name="Checking", account_group=assets, has_feed=True)
        groceries = Account.objects.create(team=cls.team, name="Groceries", account_group=expenses)
        entry = JournalEntry.objects.create(team=cls.team, entry_date=date(2026, 9, 1), description="Deposit")
        amount = Decimal("1234.50")
        JournalLine.objects.create(team=cls.team, journal_entry=entry, account=cls.checking, dr_amount=amount)
        JournalLine.objects.create(team=cls.team, journal_entry=entry, account=groceries, cr_amount=amount)

    def test_inbox_account_shows_its_balance(self):
        nav = self._nav(reverse("web_team:home", args=[self.team.slug]))
