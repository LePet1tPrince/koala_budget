...
class ComputeUnassignedTest(UnassignedFixture):
    def test_worked_example(self):
        u = compute_unassigned(self.book, SEPT, today=MID_SEPT)
    ...
    def test_card_agrees_with_the_metric(self):
        ...
        card = NetWorthService.card_data(compute_unassigned(self.book, SEPT, today=MID_SEPT))
    ...
    def test_income_due_stops_counting_once_the_month_is_over(self):
        ...
        u = compute_unassigned(self.book, SEPT, today=date(2026, 10, 5))
    ...
    def test_last_months_income_shortfall_does_not_carry(self):
        ...
        Budget.objects.create(book=self.book, month=AUG, category=self.salary, budget_amount=Decimal("5000"))
    ...
    def test_overspending_is_carried_not_taken_from_unassigned(self):
        before = compute_unassigned(self.book, SEPT, today=MID_SEPT).amount
    ...
    def test_voided_entries_do_not_count(self):
        ...
        entry.save()
        self.assertEqual(compute_unassigned(self.book, SEPT, today=MID_SEPT).amount, Decimal("1500"))
...
class DollarMapGeometryTest(UnassignedFixture):
    def test_waterfall_steps_chain_down_to_unassigned(self):
        bars = waterfall(compute_unassigned(self.book, SEPT, today=MID_SEPT))
    ...
    def test_bar_segments_cover_the_claims(self):
        bar = allocation_bar(compute_unassigned(self.book, SEPT, today=MID_SEPT, detail=True))
        self.assertEqual(
    ...
    def test_overshoot_is_over_assignment_plus_carried_overspending(self):
        ...
        other = Account.objects.create(book=self.book, name="Fun", account_group=self.groceries.account_group)
        self.post(date(2026, 9, 7), other, self.checking, "500")
        u = compute_unassigned(self.book, SEPT, today=date(2026, 10, 5), detail=True)
