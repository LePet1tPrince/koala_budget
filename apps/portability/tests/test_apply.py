...
class GoalLifecycleRoundTripTests(TestCase):
    ...
    def test_closed_goal_survives_and_every_goal_has_an_account(self):
        ...
        Goal.objects.filter(book=source_team.default_book, name="New Deck").update(closed_at=closed_at)

        apply.apply_archive(dest_team.default_book, export_bytes(source_team.default_book), user=dest_user)

        goal = Goal.objects.get(book=dest_team.default_book, name="New Deck")
