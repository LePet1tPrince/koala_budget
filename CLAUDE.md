...
- Bank feed: transfer double-count review. A transfer between two of the team's own accounts is reported by both banks, so it lands in the feed twice; categorizing both legs creates two journal entries and double-counts the movement in every balance and on the balance sheet. `apps/bank_feed/services/transfer_detection.py::find_transfer_candidates()` flags likely pairs (different account, equal magnitude + opposite direction, posted within `BANK_FEED_TRANSFER_WINDOW_DAYS` (default 5), neither archived nor voided, not previously dismissed; greedy one-to-one, closest-date wins) — covering both uncategorized *and* already-categorized pairs so existing double-counted history can be cleaned up. Endpoints on `BankFeedViewSet`: `GET transfers/` (suggestions), `POST transfers/resolve/` `{archive_id, keep_id}` (archives the duplicate leg and **voids** its journal entry so it drops out of balances via `NOT_VOID`; refuses reconciled legs), `POST transfers/dismiss/` `{transaction_a, transaction_b}` (records a `TransferMatchDismissal` so the pair stops being suggested). The kept leg is left for the user to categorize as a transfer. New `AuditEvent` types `transfer_dup_resolved`/`transfer_dup_dismissed`. Frontend: `TransferSuggestions.jsx` review panel in the bank feed with three buttons per pair (archive A / archive B / not a duplicate); transfer API helpers added to `getBatchOperationsApi` in `assets/javascript/bank_feed/bank_feed.js`.
